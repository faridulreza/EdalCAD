 ...
 *                                                                         *
 *   This file is part of the EdalCAD CAx development system.              *
 *                                                                         *
...
static PyObject *
EdalCADGui_showMainWindow(PyObject * /*self*/, PyObject *args)
{
...
static PyObject *
EdalCADGui_exec_loop(PyObject * /*self*/, PyObject *args)
{
...
static PyObject *
EdalCADGui_setupWithoutGUI(PyObject * /*self*/, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ""))
        return nullptr;
...
static PyObject *
EdalCADGui_embedToWindow(PyObject * /*self*/, PyObject *args)
{
    char* pointer;
...
struct PyMethodDef EdalCADGui_methods[] = {
    {"showMainWindow",EdalCADGui_showMainWindow,METH_VARARGS,
     "showMainWindow() -- Show the main window\n"
     "If no main window does exist one gets created"},
    {"exec_loop",EdalCADGui_exec_loop,METH_VARARGS,
     "exec_loop() -- Starts the event loop\n"
     "Note: this will block the call until the event loop has terminated"},
    {"setupWithoutGUI",EdalCADGui_setupWithoutGUI,METH_VARARGS,
     "setupWithoutGUI() -- Uses this module without starting\n"
     "an event loop or showing up any GUI\n"},
    {"embedToWindow",EdalCADGui_embedToWindow,METH_VARARGS,
    ...
    if (!Gui::MainWindow::getInstance()) {
        ...
        if (!init) {
            try {
                ...
                Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("EdalCADGuiInit"));
            }
            catch (const Base::Exception& e) {
                PyErr_Format(Base::PyExc_FC_GeneralError, "Error in EdalCADGuiInit.py: %s\n", e.what());
                return nullptr;
            }
            init = true;
        }
...
PyMOD_INIT_FUNC(EdalCADGui)
{
    try {
        Base::Interpreter().loadModule("EdalCAD");
        App::Application::Config()["AppIcon"] = "EdalCAD";
        App::Application::Config()["SplashScreen"] = "EdalCADsplash";
        App::Application::Config()["CopyrightInfo"] = "\xc2\xa9 EdalCAD\n";
        App::Application::Config()["LicenseInfo"] = "EdalCAD is free and open-source software licensed under the terms of LGPL2+ license.\n";
        App::Application::Config()["CreditsInfo"] = "EdalCAD wouldn't be possible without FreeCAD community.\n";
        ...
        static struct PyModuleDef EdalCADGuiModuleDef = {
            PyModuleDef_HEAD_INIT,
            "EdalCADGui", "EdalCAD GUI module\n", -1,
            EdalCADGui_methods,
            nullptr, nullptr, nullptr, nullptr
        };
        PyObject* module = PyModule_Create(&EdalCADGuiModuleDef);
        return module;
    }
    catch (const Base::Exception& e) {
